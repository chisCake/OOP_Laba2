...
namespace OOP_Laba2 {
    class Types {
        ...
        private static void Conversions() {
            ...
            double f = 1707.1707;
            int g = (int)f;
            Console.WriteLine($"Явно: double f {f} --(int)--> int g {g}");
            int h = Convert.ToInt32(f);
            Console.WriteLine($"Явно: double f {f} --Convert.ToInt32()--> int h {h}");
            string i = f.ToString();
            Console.WriteLine($"Явно: double f {f} --.ToString()--> string g {h}");
            bool btrue = Convert.ToBoolean(f), bfalse = Convert.ToBoolean(0);
            Console.WriteLine($"Явно: double f {f} --Convert.ToBoolean()--> bool btrue {btrue}");
            Console.WriteLine($"Явно: 0 --Convert.ToBoolean()--> bool bfalse {btrue}");
            DateTime date = Convert.ToDateTime("07.09.2020 21:55");
            Console.WriteLine($"Явно: \"07.09.2020 21:55\" --Convert.ToDateTime()--> {date}");
        }

        private static void PackingUnpacking() {
            int some_int = 2200;
            object some_object = some_int;
            Console.WriteLine($"Упаковка int: {some_int} --> {some_object}");
            string some_string = some_object.ToString();
            Console.WriteLine($"Распаковка object: {some_object} --> {some_string}");
        }
        ...
        private static void VarExample() {
            ...
            foreach (var item in list) {
                Console.WriteLine($"Перечисление элементов типа {item.GetType()}");
        ...
        private static void VarErrorExample() {
            ...
            //smth = 123.132;
            //smth = "123";
            Console.WriteLine("Переменной типа var нельзя присвоить значение отличающегося по типу от начального, " +
